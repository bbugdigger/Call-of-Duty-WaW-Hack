...
DWORD WINAPI TestHackThread(HMODULE hModule) {
	...
	Vec3* cords = (Vec3*)(hack->dwLocalPlayer + 0x20);
	int health = *(int*)(hack->dwLocalPlayer + 0x164);
	...
	while (!GetAsyncKeyState(VK_END)) {
		...
		for (int i = 0; i < 24; i++) {
			Zombie* currZombie = (Zombie*)(hack->dwEntityList + i * 0x88);
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD reason, LPVOID lpr) {
	if (reason == DLL_PROCESS_ATTACH)
		CloseHandle(CreateThread(0, 0, (LPTHREAD_START_ROUTINE)TestHackThread, hModule, 0, 0));
	return TRUE;
}